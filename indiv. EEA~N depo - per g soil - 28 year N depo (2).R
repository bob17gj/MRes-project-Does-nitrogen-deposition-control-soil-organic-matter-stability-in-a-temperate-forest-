...
AP.enzyme.plt <- ggplot(AP.ancova.model_1, aes(x = NOy.NHx.combined.28.year, y = standardised.enzyme.activity.rate))+
  ...
  labs(title="AP")+
  xlab(expression(paste('N depositon (kg ha'^-1," yr"^-1,")")))+  ylab(expression(atop("Enzyme activity",paste(" (",mu,'mol g'^-1," soil h"^-1,")"))))+
  ...
  theme_classic() +   
  theme(axis.text=element_text(size = 15), axis.title=element_text(size = 20),legend.position="none",legend.title = element_text(size = 15), legend.text=element_text(size = 15))
...
BX.enzyme.plt <- ggplot(BX.ancova.model_1, aes(x = NOy.NHx.combined.28.year, y = standardised.enzyme.activity.rate, colour = horizon))+
  ...
  labs(shape = "Forest Type") + labs(colour = "Horizon") +
  labs(title="BX")+
  xlab(expression(paste("")))+      ylab(expression(atop("")))+
  scale_x_continuous(breaks=c(0,5,10,15,20,25,30,35))+
  expand_limits(x=c(5,35))+
  scale_y_continuous()+
...
PPO.enzyme.plt <- ggplot(PPO.ancova.model_1, aes(x = NOy.NHx.combined.28.year, y = standardised.enzyme.activity.rate))+
  ...
  labs(shape = "Forest Type") + labs(colour = "Horizon") +
  labs(title="PPO")+
  xlab(expression(paste('N depositon (kg ha'^-1," yr"^-1,")")))+  ylab(expression(atop("")))+
  scale_x_continuous(breaks=c(0,5,10,15,20,25,30,35))+
  scale_y_continuous()+ 
...
NAG.enzyme.plt <- ggplot(NAG.ancova.model_1, aes(x = NOy.NHx.combined.28.year, y = standardised.enzyme.activity.rate, colour = horizon))+
  ...
  labs(shape = "Forest Type") + labs(colour = "Horizon") +
  labs(title="NAG")+
  xlab(expression(paste("")))+         ylab(expression(atop("Enzyme activity",paste(" (",mu,'mol g'^-1," soil h"^-1,")"))))+
  scale_x_continuous(breaks=c(0,5,10,15,20,25,30,35))+
  expand_limits(x=c(5,35))+
  scale_y_continuous()+ 
  ...
  scale_colour_manual(name="Horizon", # Legend label, use darker colors
                      ...
                      labels=c("A", "O"),
                      values=c("#CC9900","#990000"))+
  theme_classic() +   theme(axis.text=element_text(size = 15), axis.title=element_text(size = 20),legend.position="none",legend.title = element_text(size = 15), legend.text=element_text(size = 15))
...
EEA.Ndepo.plot <- 
  plot_grid(BG.enzyme.plt, BX.enzyme.plt, NAG.enzyme.plt, LAP.enzyme.plt, AP.enzyme.plt, PPO.enzyme.plt, 
          nrow = 3, labels = c("a   ", "b    ","c    ", "d    ", "e    ", "f    "), label_size = 17)

EEA.Ndepo.plot

ggsave("EEA~N_depo1_____.pdf", height = 12, width = 13)
